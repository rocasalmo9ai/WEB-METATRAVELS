...
export const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="flex flex-col min-h-screen font-sans bg-neutral-950">
      ...
      {/* Dynamic Header - Safe-area + fixed overlap hardening */}
      <header
        className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-700 ease-in-out pt-[env(safe-area-inset-top)] ${
          isScrolled
            ? 'bg-black/95 backdrop-blur-2xl shadow-[0_20px_80px_-10px_rgba(0,0,0,1)] h-[calc(5rem+env(safe-area-inset-top))] border-b border-white/5'
            : 'bg-gradient-to-b from-black/90 via-black/40 to-transparent h-[calc(7rem+env(safe-area-inset-top))]'
        }`}
      >
        <div className="container mx-auto px-8 h-full">
          <div className="flex items-center justify-between h-full">
            ...
            <Link to="/" className="flex items-center group relative z-10">
              <div
                className={`font-bold tracking-tighter flex items-center transition-all duration-500 drop-shadow-[0_4px_12px_rgba(0,0,0,1)] ${
              ...
              >
                ...
                <span className="text-accent relative flex items-center ml-1.5">
                  Travels
                  <svg
                    className={`absolute -top-1 -right-7 transform rotate-45 text-accent opacity-0 group-hover:opacity-100 transition-all ${
                      isScrolled ? 'w-5 h-5 -right-6' : 'w-7 h-7'
                    }`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
            ...
            <nav className="hidden lg:flex items-center space-x-12">
              {navLinks.map((link) => (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`text-[11px] font-black transition-all uppercase tracking-[0.3em] relative group drop-shadow-[0_4px_8px_rgba(0,0,0,1)] ${
            ...
            <div className="hidden md:flex items-center space-x-10">
              ...
              <Link
                to="/custom-trip"
                className={`bg-accent hover:bg-accent-hover text-white rounded-full text-[10px] font-black uppercase tracking-[0.25em] transition-all transform hover:scale-105 active:scale-95 shadow-[0_15px_40px_rgba(245,158,11,0.3)] border border-accent/30 ${
        ...
        {/* Mobile Menu Overlay */}
        <div
          className={`fixed inset-0 bg-neutral-950 z-40 lg:hidden transition-transform duration-700 ease-in-out transform ${
            isMenuOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
        >
          <div className="flex flex-col h-full p-12 pt-[calc(8rem+env(safe-area-inset-top))]">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
      ...
      {/* Main Content - push below fixed header (incl. safe-area) */}
      <main className="flex-grow pt-[calc(7rem+env(safe-area-inset-top))]">{children}</main>
        ...
        <div className="container mx-auto px-8 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-20 mb-28">
            <div className="space-y-10">
              ...
              <p className="text-neutral-500 font-light leading-relaxed text-base">{t.footer.desc}</p>
              <div className="flex space-x-6">
                <a
                  href={SITE_CONFIG.socials.instagram}
                  className="w-12 h-12 rounded-2xl border border-white/10 flex items-center justify-center text-neutral-400 hover:text-accent hover:border-accent hover:scale-110 transition-all"
                >
                  <Instagram size={22} />
                </a>
                <a
            ...
            <div>
              ...
              <ul className="space-y-6">
                <li>
                  <Link to="/packages" className="text-neutral-400 hover:text-white transition-colors text-sm font-bold uppercase tracking-widest">
                    {t.nav.destinations}
                  </Link>
                </li>
                <li>
                  <Link to="/weather" className="text-neutral-400 hover:text-white transition-colors text-sm font-bold uppercase tracking-widest">
                    {t.nav.weather}
                  </Link>
                </li>
                <li>
                ...
                <li>
                  <Link to="/custom-trip" className="text-neutral-400 hover:text-white transition-colors text-sm font-bold uppercase tracking-widest">
                    {t.nav.customTrip}
                  </Link>
                </li>
              </ul>
            ...
            <div>
              ...
              <ul className="space-y-6">
                <li>
                  <Link to="/policy" className="text-neutral-400 hover:text-white transition-colors text-sm font-bold uppercase tracking-widest">
                    {t.footer.terms}
                  </Link>
                </li>
                <li>
            ...
            <div>
              ...
              <ul className="space-y-8">
                <li className="flex items-start">
                  <div className="bg-neutral-900 border border-white/5 p-3 rounded-2xl mr-5">
          ...
          <div className="border-t border-white/5 pt-12 flex flex-col md:flex-row justify-between items-center gap-10">
            <p className="text-neutral-600 text-[11px] font-bold uppercase tracking-[0.2em]">
              © {new Date().getFullYear()} Meta Travels Group. {t.footer.crafted}
            </p>
            <div className="flex gap-8">
              <span className="text-neutral-700 text-[10px] font-black uppercase tracking-[0.3em]">{t.footer.inspiring}</span>
              <span className="text-neutral-700 text-[10px] font-black uppercase tracking-[0.3em] px-4 border-l border-white/5">
                {t.footer.premium}
              </span>
            </div>
          </div>
