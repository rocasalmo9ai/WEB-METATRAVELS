...
export const PackageDetail: React.FC = () => {
  ...
  if (!pkg) {
    return <div className="p-20 text-center text-white bg-neutral-900 min-h-screen flex items-center justify-center text-3xl font-serif">{t.packageDetail.notFound}</div>;
  }
    ...
    <div className="animate-fade-in pb-20 bg-white">
        ...
        <div className="absolute bottom-0 left-0 w-full p-8 md:p-16 text-white bg-gradient-to-t from-black/90 to-transparent">
          <div className="container mx-auto">
            ...
            <div className="flex flex-wrap gap-6 mt-6 text-sm md:text-base font-medium">
               <span className="flex items-center"><Calendar className="w-5 h-5 mr-2 text-accent"/> {getText(pkg.dates)}</span>
               <span className="flex items-center"><MapPin className="w-5 h-5 mr-2 text-accent"/> {getText(pkg.duration)}</span>
               <span className="flex items-center"><Users className="w-5 h-5 mr-2 text-accent"/> {t.packageDetail.groupSize} {pkg.minGroupSize}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 mt-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-16">
          ...
          {/* Intro Description */}
          <section>
            <h2 className="text-3xl font-bold text-gray-900 mb-6 border-l-4 border-accent pl-4">{t.packageDetail.description}</h2>
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4">
               {pkg.highlights.map((h, i) => (
                 <div key={i} className="flex items-center bg-purple-50 p-4 rounded-lg">
                   <div className="bg-white p-2 rounded-full mr-3 shadow-sm text-primary"><Camera size={18} /></div>
          ...
          {/* Itinerary */}
          <section>
            <h2 className="text-3xl font-bold text-gray-900 mb-8 border-l-4 border-accent pl-4">{t.packageDetail.itinerary}</h2>
            <div className="space-y-12">
              {pkg.itinerary.map((day) => (
                <div key={day.day} className="bg-white rounded-none md:rounded-2xl overflow-hidden shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                  {day.image && (
                  ...
                  <div className="p-8">
                    {!day.image && <span className="text-primary font-bold text-xl block mb-2">{t.packageDetail.day} {day.day}</span>}
