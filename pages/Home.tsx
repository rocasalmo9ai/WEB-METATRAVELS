...
import { ArrowRight, Star, Shield, Heart, Plane, Hotel, Map, User, Utensils, Calendar, TicketCheck, Globe, CheckCircle2, Sparkles, BrainCircuit } from 'lucide-react';
...
export const Home: React.FC = () => {
    ...
    <div className="flex flex-wrap gap-3 mt-4 pt-4 border-t border-white/5">
      {pkg.amenities.flightsIntl && (
        <div className="group relative">
           ...
           <span className="absolute bottom-full left-1/2 -translate-x-1/2 bg-black text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap mb-1">Vuelos Intl</span>
        </div>
      )}
      {pkg.amenities.flightsDomestic && (
        <div className="group relative">
           <Plane size={16} className="text-gray-500 group-hover:text-accent transition-colors" />
           <span className="absolute bottom-full left-1/2 -translate-x-1/2 bg-black text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap mb-1">Vuelos Nac</span>
        </div>
      )}
      {pkg.amenities.accommodation && (
        <div className="group relative">
           <Hotel size={16} className="text-gray-500 group-hover:text-accent transition-colors" />
           <span className="absolute bottom-full left-1/2 -translate-x-1/2 bg-black text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap mb-1">Hospedaje</span>
        </div>
      )}
      {pkg.amenities.meals && (
        <div className="group relative">
           <Utensils size={16} className="text-gray-500 group-hover:text-accent transition-colors" />
           <span className="absolute bottom-full left-1/2 -translate-x-1/2 bg-black text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap mb-1">Alimentos</span>
        </div>
      )}
      {pkg.amenities.tours && (
        <div className="group relative">
           <Map size={16} className="text-gray-500 group-hover:text-accent transition-colors" />
           <span className="absolute bottom-full left-1/2 -translate-x-1/2 bg-black text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap mb-1">Tours</span>
        </div>
      )}
      {pkg.amenities.guide && (
        <div className="group relative">
           <User size={16} className="text-gray-500 group-hover:text-accent transition-colors" />
           <span className="absolute bottom-full left-1/2 -translate-x-1/2 bg-black text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap mb-1">Guía</span>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="animate-fade-in bg-neutral-900 overflow-x-hidden">
      {/* Hero Section - Imagen ahora clara y natural */}
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0 z-0">
          ...
          />
          {/* Overlay minimalista para contraste local del texto sin oscurecer la foto */}
        ...
        <div className="container mx-auto px-6 relative z-10 text-center">
          <div className="inline-flex items-center gap-3 px-6 py-2 mb-8 rounded-full border border-white/40 bg-black/30 backdrop-blur-xl animate-fade-in">
            ...
            <span className="text-[10px] font-bold uppercase tracking-[0.4em] text-white">{t.hero.defining}</span>
          </div>
          <h1 className="text-7xl md:text-[10rem] font-bold mb-10 tracking-tighter leading-none whitespace-pre-line font-serif animate-slide-up drop-shadow-[0_4px_12px_rgba(0,0,0,0.6)]">
            ...
            <span className="text-gradient filter drop-shadow-[0_4px_10px_rgba(139,92,246,0.3)]">{t.hero.titlePart2}</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-8 animate-fade-in [animation-delay:0.6s]">
            <Link to="/custom-trip" className="bg-accent hover:bg-accent-hover text-white px-16 py-6 rounded-full text-sm font-bold transition-all transform hover:scale-105 shadow-[0_15px_30px_-12px_rgba(245,158,11,0.6)] tracking-[0.3em] uppercase border border-accent/20">
              {t.hero.ctaPrimary}
            </Link>
            <Link to="/packages" className="bg-black/30 hover:bg-black/50 backdrop-blur-md text-white border border-white/40 px-16 py-6 rounded-full text-sm font-bold transition-all tracking-[0.3em] uppercase shadow-lg">
              {t.hero.ctaSecondary}
            </Link>
          </div>
        </div>
        ...
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-3 opacity-60">
           <span className="text-[8px] font-bold uppercase tracking-[0.5em] text-white drop-shadow-sm">{t.hero.scroll}</span>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
              <div className="relative">
                 ...
                 <div className="relative z-10 bg-neutral-900 p-12 rounded-[3rem] border border-white/10 shadow-3xl">
                    ...
                    <h3 className="text-4xl font-serif text-white mb-6 leading-tight">Arquitectura Emocional Meta</h3>
                    <p className="text-neutral-500 leading-relaxed text-lg font-light mb-8">
                      No solo elegimos destinos; decodificamos tu arquitectura emocional para orquestar travesías que resuenan con tu momento vital. 
                    </p>
              ...
              <div className="space-y-10">
                 ...
                 <h2 className="text-5xl md:text-6xl font-bold text-white font-serif leading-tight">El viaje comienza en tu mente.</h2>
                 <p className="text-neutral-400 text-xl font-light leading-relaxed">
                   Nuestra tecnología propietaria fusiona el análisis de datos de élite con la intuición humana para crear el itinerario perfecto, eliminando la fatiga de decisión y dejando solo la maravilla.
                 </p>
                 <Link to="/custom-trip" className="inline-flex items-center text-white font-bold tracking-[0.2em] uppercase text-xs group">
                    Comenzar Diagnóstico <ArrowRight className="ml-4 group-hover:translate-x-3 transition-transform text-accent" />
                 </Link>
              </div>
           </div>
        </div>
              ...
              <Link to={`/package/${pkg.id}`} key={pkg.id} className="group block h-full">
                <div className="relative overflow-hidden rounded-[3rem] bg-neutral-800/40 border border-white/5 hover:border-accent/30 transition-all duration-700 flex flex-col h-full shadow-2xl">
                  ...
                  <div className="relative aspect-[3/4] overflow-hidden shrink-0">
                    <img src={pkg.heroImage} alt={getText(pkg.title)} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110" />
                  ...
                  <div className="p-10 flex flex-col flex-grow">
                    ...
                    <div className="flex justify-between items-center mt-10 pt-8 border-t border-white/5">
                      <div>
                         ...
                         <p className="text-3xl font-bold text-white tracking-tighter">${pkg.price.toLocaleString()} <span className="text-xs font-light text-gray-500">{pkg.currency}</span></p>
                      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-32 items-center">
             <div className="relative">
                <div className="aspect-[4/5] rounded-[4rem] overflow-hidden shadow-3xl">
                   <img src="https://images.unsplash.com/photo-1542744173-8e7e53415bb0?q=80&w=2070&auto=format&fit=crop" className="w-full h-full object-cover grayscale brightness-50 transition-all duration-1000 hover:grayscale-0 hover:brightness-100" alt="Office Culture" />
                </div>
             ...
             <div>
                ...
                <h2 className="text-6xl md:text-7xl font-bold mt-8 mb-12 font-serif text-white tracking-tight">{t.home.philosophy.titlePart1} <br/><span className="text-gradient">{t.home.philosophy.titlePart2}</span></h2>
      ...
      <section className="py-48 relative bg-neutral-900 overflow-hidden">
          <div className="absolute inset-0 z-0">
             <img src="https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?ixlib=rb-4.0.3&auto=format&fit=crop&w=2021&q=80" className="w-full h-full object-cover opacity-10 scale-110" alt="Adventure Background" />
             <div className="absolute inset-0 bg-gradient-to-t from-neutral-900 via-transparent to-neutral-900"></div>
          </div>
          <div className="container mx-auto px-6 relative z-10 text-center">
             <h3 className="text-6xl md:text-8xl font-bold mb-10 text-white font-serif max-w-5xl mx-auto leading-none tracking-tighter">{t.home.customTripTitle}</h3>
             <p className="mb-16 text-xl md:text-3xl text-gray-400 max-w-3xl mx-auto font-light leading-relaxed">{t.home.customTripDesc}</p>
             <Link to="/custom-trip" className="inline-block bg-white text-black px-20 py-7 rounded-full font-bold transition-all shadow-3xl hover:bg-accent hover:text-white hover:scale-105 active:scale-95 tracking-[0.3em] uppercase text-sm">
